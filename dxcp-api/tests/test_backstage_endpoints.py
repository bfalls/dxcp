...
async def test_delivery_status_latest_deployment(tmp_path: Path):
    async with _client_and_state(tmp_path, "OBSERVER") as (client, main):
        _insert_deployment(main.storage, "dep-1", "SUCCEEDED")
        response = await client.get("/v1/services/demo-service/delivery-status")
...
async def test_allowed_actions_observer(tmp_path: Path):
    async with _client_and_state(tmp_path, "OBSERVER") as (client, _):
        response = await client.get("/v1/services/demo-service/allowed-actions")
    assert response.status_code == 200
    body = response.json()
