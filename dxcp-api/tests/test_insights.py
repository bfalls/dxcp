...
async def test_insights_aggregation(tmp_path: Path):
    async with _client_and_state(tmp_path) as (client, main):
        ...
        response = await client.get("/v1/insights/failures?windowDays=30")
